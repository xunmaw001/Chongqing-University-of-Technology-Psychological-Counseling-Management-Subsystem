...
<body>
        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.laoshiName}}</cite></a>
                </span>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.laoshiPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.laoshiUuidNumber">心理咨询老师编号：
                                {{detail.laoshiUuidNumber}}
                            </li>
                            <li v-if="detail.laoshiPhone">心理咨询老师手机号：
                                {{detail.laoshiPhone}}
                            </li>
                            <li v-if="detail.laoshiShanchang">擅长：
                                {{detail.laoshiShanchang}}
                            </li>
                            <li v-if="detail.sexTypes">性别：
                                {{detail.sexValue}}
                            </li>
                            <li v-if="detail.laoshiEmail">电子邮箱：
                                {{detail.laoshiEmail}}
                            </li>
                            <%--<li v-if="detail.laoshiText">心理咨询老师介绍：
                                {{detail.laoshiText}}
                            </li>--%>
                        </ul>
                        ...
                        心理咨询老师介绍：
                        <div class="blog-details-content" v-html="myFilters(detail.laoshiText)">
                        </div>
        ...
        <!-- 座位 -->
                                ...
                                <button onclick="addLaoshiOrder()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                    立即购买
                                </button>
                            </div>
                        </div>--%>
                        <div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                            <div class="num-picker">
                                <button @click="laoshiYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                    立即预约
                                </button>
                            </div>
                        </div>
        ...
        <div class="col-lg-4" v-if="laoshiRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in laoshiRecommendList" v-bind:key="index" @click="jump('../laoshi/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.laoshiPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.laoshiName}}</a>
                                <h6 v-if="item.laoshiNewMoney" class="post-title"><a style="color: red">{{item.laoshiNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    // 立即购买
    function addLaoshiOrder(){
        localStorage.setItem("laoshiId",vue.detail.id);
        localStorage.setItem("buyNumber",vue.buyNumber);
        window.location.href='../laoshiOrder/add.jsp';
    }
