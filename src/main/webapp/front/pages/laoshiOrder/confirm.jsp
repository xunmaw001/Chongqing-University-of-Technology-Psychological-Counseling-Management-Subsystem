		...
		<script>
			var vue = new Vue({
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.NewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						let data={
                            laoshiId: localStorage.getItem('userid'),
                            xueshengId: localStorage.getItem('userid'),
						}
                        // 获取商品详情信息
                        layui.http.request(`laoshiOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../laoshiOrder/list.jsp';
							}
                       		 window.location.href='../laoshiOrder/list.jsp';
					    });
					}
